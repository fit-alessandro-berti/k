...
class DFG
  ...
  def project(activities_subset)
    ...
    projected_start_activities = @start_activities & activities_subset
    projected_end_activities = @end_activities & activities_subset
...
def detect_parallel_cut(dfg)
  # Implementation of parallel cut detection
  groups = []
  activities = dfg.activities.to_a

  while !activities.empty?
    group = Set.new([activities.shift])
    added = true
    while added
      added = false
      rest_activities = activities - group.to_a
      rest_activities.each do |activity|
        interconnections = group.any? do |g_activity|
          dfg.edges.include?([activity, g_activity]) && dfg.edges.include?([g_activity, activity])
        end
        unless interconnections
          group << activity
